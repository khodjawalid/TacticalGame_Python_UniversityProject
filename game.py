...
class Game:
    def __init__(self, screen):
        ...
        self.player_units = [
            Type_Unite("Alex", 0, 0, 100, 30, "player", 10, 1, [competence_soin], "./A.jpg"),
            Type_Unite("Clara", 0, 2, 100, 25, "player", 15, 2, [competence_attaque_puissante], "./B.jpg"),
            Type_Unite("Maxime", 0, 3, 100, 35, "player", 10, 3, [competence_attaque_puissante], "./C.jpg"),
            Type_Unite("Sophie", 0, 1, 100, 20, "player", 20, 4, [competence_soin], "./D.jpg"),
        ]

        self.enemy_units = [
            Type_Unite("Alex", 14, 14, 100, 30, "enemy", 10, 1, [competence_soin], "./A.jpg"),
            Type_Unite("Clara", 14, 13, 100, 25, "enemy", 15, 2, [competence_attaque_puissante], "./B.jpg"),
            Type_Unite("Maxime", 14, 12, 100, 35, "enemy", 10, 3, [competence_attaque_puissante], "./C.jpg"),
            Type_Unite("Sophie", 14, 11, 100, 20, "enemy", 20, 4, [competence_soin], "./D.jpg"),
        ]
    ...
    def handle_enemy_turn(self):
        ...
        for selected_unit in self.enemy_units:
            ...
            nearest_player = self.get_nearest_player(selected_unit)

            dx, dy = 0, 0
            if selected_unit.x < nearest_player.x:
                dx = 1
            elif selected_unit.x > nearest_player.x:
                dx = -1

            if selected_unit.y < nearest_player.y:
                dy = 1
            elif selected_unit.y > nearest_player.y:
                dy = -1
            ...
            # Attaque si l'ennemi est proche
            if abs(selected_unit.x - nearest_player.x) <= 1 and abs(selected_unit.y - nearest_player.y) <= 1:
                selected_unit.attack(nearest_player)
    ...
    def flip_display(self):
        ...
        # Afficher uniquement les joueurs sélectionnés
        for unit in self.player_units + self.enemy_units:
    ...
    def afficher_tableau(self):
        ...
        pygame.draw.rect(self.screen, BLACK, tableau_rect)

        # Texte des scores et tours
        score_text = font.render(
            f"Tour: {self.tour} | Player: {self.player_score} - {self.enemy_score}   : Enemy", True, WHITE
        )
...
def main():
    ...
    while True:
        ...
        if action == "Solo":
            ...
            while True:
                accessible_cells = game.get_accessible_cells(selected_player)
                game.draw_accessible_cells(accessible_cells)
                pygame.display.flip()
